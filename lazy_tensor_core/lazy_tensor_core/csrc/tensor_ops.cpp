...
LazyTensor SmoothL1Loss(const LazyTensor& input, const LazyTensor& target,
  ...
  auto broadcasted_inputs =
      lazy_tensor_aten_ops::broadcast_tensors({input, target});
  ...
  LazyTensor diff =
      lazy_tensor_aten_ops::sub(broadcasted_input, broadcasted_target, one);
  at::Scalar half(0.5);
  at::Scalar half_beta(0.5 * beta);
  LazyTensor abs_diff = lazy_tensor_aten_ops::abs(diff);
  LazyTensor squared_loss = lazy_tensor_aten_ops::div(
      lazy_tensor_aten_ops::mul(lazy_tensor_aten_ops::mul(diff, diff), half),
      beta_scalar);
  LazyTensor l1_loss = lazy_tensor_aten_ops::sub(abs_diff, half_beta, one);
  LazyTensor elementwise_loss = lazy_tensor_aten_ops::where(
      lazy_tensor_aten_ops::lt(abs_diff, beta_scalar), squared_loss, l1_loss);
  ...
  switch (reduction) {
    ...
    case ReductionMode::kSum:
      return lazy_tensor_aten_ops::sum(elementwise_loss, all_dimensions, false,
                                       broadcasted_input.dtype());
    default:
...
LazyTensor SmoothL1LossBackward(const LazyTensor& grad_output,
  ...
  LazyTensor diff =
      lazy_tensor_aten_ops::sub(broadcasted_input, broadcasted_target, one);
  LazyTensor abs_diff = lazy_tensor_aten_ops::abs(diff);
  LazyTensor grad_squared_loss = lazy_tensor_aten_ops::div(
      lazy_tensor_aten_ops::sub(broadcasted_input, broadcasted_target, one),
      beta_scalar);
  LazyTensor ones = lazy_tensor_aten_ops::full_like(
      broadcasted_input, one, broadcasted_input.GetDevice(),
      broadcasted_input.dtype());
  // NB: We can't use lazy_tensor_aten_ops::sign(), it returns zero for input
  // zero.
  LazyTensor grad_l1_loss = lazy_tensor_aten_ops::where(
      lazy_tensor_aten_ops::gt(broadcasted_input, broadcasted_target), ones,
      lazy_tensor_aten_ops::neg(ones));
  LazyTensor elementwise_loss_backward = lazy_tensor_aten_ops::where(
      lazy_tensor_aten_ops::lt(abs_diff, beta_scalar), grad_squared_loss,
      grad_l1_loss);
  switch (reduction) {
    case ReductionMode::kNone:
    case ReductionMode::kSum:
      return lazy_tensor_aten_ops::mul(elementwise_loss_backward, grad_output);
    case ReductionMode::kMean: {
      LazyTensor grad_scale = lazy_tensor_distributed::get_dimensions_size(
          broadcasted_input,
          Helpers::GetAllDimensions(broadcasted_input.shape()));
      return lazy_tensor_aten_ops::mul(
          lazy_tensor_aten_ops::div(elementwise_loss_backward, grad_scale),
          grad_output);
    }
    default:
      LTC_ERROR() << "Invalid reduction type: "
...
LazyTensor Softplus(const LazyTensor& input, const at::Scalar& beta,
  ...
  return lazy_tensor_aten_ops::where(
      lazy_tensor_aten_ops::gt(lazy_tensor_aten_ops::mul(input, beta),
                               threshold),
      input,
      lazy_tensor_aten_ops::div(
          lazy_tensor_aten_ops::log1p(lazy_tensor_aten_ops::exp(
              lazy_tensor_aten_ops::mul(input, beta))),
          beta));
}
...
LazyTensor SoftplusBackward(const LazyTensor& grad_output,
  ...
  LazyTensor scaled_input = lazy_tensor_aten_ops::mul(input, beta);
  LazyTensor z =
      lazy_tensor_aten_ops::exp(lazy_tensor_aten_ops::mul(output, beta));
  return lazy_tensor_aten_ops::where(
      lazy_tensor_aten_ops::gt(scaled_input, threshold), grad_output,
      lazy_tensor_aten_ops::mul(
          grad_output,
          lazy_tensor_aten_ops::div(lazy_tensor_aten_ops::sub(z, 1, 1), z)));
}
